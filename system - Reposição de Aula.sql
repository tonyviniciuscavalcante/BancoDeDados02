...
/*
1. Acrescente as seguintes colunas:
ALTER TABLE atleta ADD idade NUMBER;
ALTER TABLE modalidade ADD qtde_praticantes NUMBER;

Execute os comandos para atualizar essas novas colunas
com os valores corretos.

UPDATE atleta SET idade = TRUNC(months_between(sysdate,
datanasc)/12);
UPDATE modalidade m
SET qtde_praticantes = (SELECT COUNT(*) FROM pratica
WHERE id_modalidade = m.id);

-- Se der erro em triggers da aula anterior na tabela MODALIDADE, desabilite-as:
ALTER TABLE modalidade DISABLE ALL TRIGGERS;
*/

CREATE OR REPLACE TRIGGER trg_update_idade
    BEFORE INSERT OR UPDATE
    ON atleta
    FOR EACH ROW
BEGIN
    IF :NEW.datanasc IS NOT NULL THEN
        :NEW.idade := TRUNC(months_between(SYSDATE, :NEW.datanasc) / 12);
    END IF;
END;


CREATE OR REPLACE TRIGGER trg_update_qtde_praticantes
    AFTER INSERT OR DELETE
    ON pratica
    FOR EACH ROW
BEGIN
    UPDATE modalidade m
    SET m.qtde_praticantes = (SELECT COUNT(*)
                              FROM pratica
                              WHERE id_modalidade = m.id);
END;
...
-- Exercicios --
/*
 1. Crie um trigger que atualize automaticamente a coluna
idade dos atletas sempre que uma tupla for inserida ou
atualizada na tabela ATLETA. Utilize a seguinte fórmula
para atualizar a idade: trunc(months_between(sysdate,
datanasc)/12)
 */
...
/*
 2. Crie um trigger que deve atualizar a quantidade de
praticantes de uma modalidade toda vez que uma nova
tupla for inserida, atualizada ou removida na tabela
PRATICA.
 */
...
CREATE OR REPLACE TRIGGER trg_update_qtde_praticantes
    ...
    FOR EACH ROW
BEGIN
    -- Atualiza a quantidade de praticantes na modalidade do registro inserido ou atualizado
    UPDATE modalidade m
    SET m.qtde_praticantes = (SELECT COUNT(*)
                              FROM pratica p
                              WHERE p.id_modalidade = m.id)
    WHERE m.id = :OLD.id_modalidade
       OR m.id = :NEW.id_modalidade;
END;

INSERT INTO pratica (id_atleta, id_modalidade)
VALUES (1, 2);
...
/*
 3. Faça uma função chamada FU_GET_TOTAL_CAMP que
receba por parâmetro o nome de um atleta. Calcule a
quantidade de campeonatos que esse atleta já disputou e
retorne-a na função.

No final, chame a função FU_GET_TOTAL_CAMP passando
o nome de um atleta por parâmetro para testar, ou chame a
função para todos os atletas.

Exemplo de execução:

SELECT FU_GET_TOTAL_CAMP('Heinrick Samter') AS QTDE_CAMPEONATOS
FROM dual;

ou

SELECT nome, FU_GET_TOTAL_CAMP(nome) AS QTDE_CAMPEONATOS
FROM empregado;
 */
...
(
    id_atleta     NUMBER(4),
    id_campeonato NUMBER(4),
    CONSTRAINT participa_pk PRIMARY KEY (id_atleta, id_campeonato),
    CONSTRAINT participa_atleta_fk FOREIGN KEY (id_atleta) REFERENCES atleta (id),
    CONSTRAINT participa_campeonato_fk FOREIGN KEY (id_campeonato) REFERENCES campeonato (id)
);

CREATE OR REPLACE FUNCTION FU_GET_TOTAL_CAMP(p_nome_atleta VARCHAR2)
    RETURN NUMBER
    IS
    v_qtde_campeonatos NUMBER := 0;
BEGIN
    SELECT COUNT(*)
    INTO v_qtde_campeonatos
    FROM participa p
             JOIN atleta a ON p.id_atleta = a.id
    WHERE a.nome = p_nome_atleta;

    RETURN v_qtde_campeonatos;
EXCEPTION
...
/*
 4. Faça uma procedure chamada PR_CALC_PREMIO_ATL_CLUBE
que receba por parâmetro o nome de um clube. Calcule o total
em premiação recebida por cada atleta desse clube, imprimindo
na tela o nome e ID dos atletas e o total recebido em premiação.
No final imprima também o valor total em premiação recebida
pelo clube somando os prêmios individuais de seus atletas.
Exemplo de execução:

EXEC PR_CALC_PREMIO_ATL_CLUBE('Tamplight Club');
Atleta 6 - Ranice Sherbourne: $0.00
Atleta 12 - Sophie Teodorski: $0.00
Atleta 18 - Madella Musgrove: $0.00
Atleta 35 - Paula Wotton: $6,928.72
Atleta 39 - Hazel Rheubottom: $14,597.76
Atleta 67 - Neel Rootham: $0.00
Atleta 74 - Olwen Dellenbrok: $0.00
Total em premiação dos atletas do clube: $21,526.48
 */
...
(
    id_atleta NUMBER(4),
    valor     NUMBER(10, 2),
    CONSTRAINT premiacao_atleta_fk FOREIGN KEY (id_atleta) REFERENCES atleta (id)
);

CREATE OR REPLACE PROCEDURE PR_CALC_PREMIO_ATL_CLUBE(p_nome_clube VARCHAR2)
    IS
    v_total_clube  NUMBER := 0;
    v_total_atleta NUMBER := 0;
...
BEGIN
    FOR r_atleta IN c_atletas
        LOOP
            ...
            INTO v_total_atleta
            FROM premiacao p
            WHERE p.id_atleta = r_atleta.id;

            DBMS_OUTPUT.PUT_LINE('Atleta ' || r_atleta.id || ' - ' || r_atleta.nome || ': $' ||
                                 TO_CHAR(v_total_atleta, '999,999.99'));

            v_total_clube := v_total_clube + v_total_atleta;
        END LOOP;
