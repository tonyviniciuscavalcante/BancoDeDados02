...
GRANT INSERT, SELECT ON teste_priv2 TO JOSE;
...
UPDATE atleta SET salario = 8000
...
VALUES (199, 'Rebeca Andrade', '888-77-5555', 2500);

COMMIT;

UPDATE atleta SET salario = 4000 WHERE id = 199;

GRANT SELECT ON clube TO JOSE;

UPDATE atleta SET salario = 98000 WHERE nome = 'Rebeca Andrade';
...
UPDATE atleta SET salario = 7200 WHERE nome = 'Oren Peers';
UPDATE atleta SET salario = 3800 WHERE nome = 'Johna Belf';

SELECT * FROM atleta WHERE nome IN ('Oren Peers', 'Johna Belf');

SAVEPOINT ATUALIZA;

UPDATE atleta SET salario = 100000 WHERE nome = 'Rebeca Andrade';
SELECT * FROM atleta WHERE nome = 'Rebeca Andrade';
-- ERROU O SALARIO!
...
UPDATE atleta SET salario = 7200 WHERE nome = 'Lambert Taffs';

COMMIT;

SELECT salario FROM atleta WHERE nome = 'Lambert Taffs';


/* ====================================== EXERCÍCIO ======================================*/

/*1. Crie uma tabela chamada EMP_TEMP no esquema HR com os campos: COD
number(2) PK, NOME varchar2(20), SAL number(6,2)*/
...
/*2. Conceda privilégio de consulta, inserção, atualização e deleção na tabela
HR.EMP_TEMP ao JOSE.*/

GRANT  select, insert, update, delete on EMP_TEMP to JOSE;


/*4. Como HR, insira 2 registros na tabela EMP_TEMP*/

insert into EMP_TEMP (COD, NOME, SAL) values (1, 'Tonyn', 1800);
insert into EMP_TEMP (COD, NOME, SAL) values (2, 'Ale Muralha', 2800);

/*6. Como HR, efetive permanentemente as inserções realizadas e depois, como
JOSE, consulte novamente os registros de EMP_TEMP.*/

COMMIT;

/*7. Como HR, insira outros 2 registros na tabela EMP_TEMP.*/

insert into EMP_TEMP (COD, NOME, SAL) values (3, 'Vitin', 3000);
insert into EMP_TEMP (COD, NOME, SAL) values (4, 'Crazy Frog', 7800);

/*9. Como HR, crie a tabela DEP_TEMP em seu esquema, com os campos: NUM
number(2) PK, NOME varchar2(20).*/
...
Depois, como JOSE, faça um SELECT consultando as informações do empregado
alterado. Qual salário JOSE enxerga? Por quê?*/


update EMP_TEMP set sal = sal*2 where COD in (1,3);
/*Após atualização o salário para JOSE não muda pois ainda não efetuamos o commit*/

/*12. Como HR, desfaça a alteração que dobrava o salário do empregado e consulte
os dados do empregado. Então, como JOSE, consulte também os dados desse
empregado.
*/
...
update EMP_TEMP set sal = 350 where COD = 2;

-- Jose ficou em looping esperando a transação do hr terminar

/*19. Como HR, desfaça a última alteração de salário feita. O que aconteceu em cada
uma das sessões agora? Faça um SELECT no registro alterado nas duas sessões e
compare.*/

ROLLBACK;
select * from emp_temp;
-- a sessão terminou e ambas as transações foram executadas.
