...
INSERT INTO tabela_teste (numero, descricao) VALUES (teste_seq.NEXTVAL, 'Tony');
...
INSERT INTO clube (id, nome)VALUES (clube_seq.nextval, 'Tabajara');

SELECT clube_seq.currval FROM dual;
...
SELECT id, datanasc, endereco, salario FROM ATLETA;

SELECT id as ID_CLUBE, nome AS NOME_CLUBE, id_presidente as PRESIDENTE FROM CLUBE;

SELECT nome, salario FROM ATLETA WHERE nome like 'R%';

SELECT nome, sexo FROM atleta WHERE nome like '%t_';

SELECT nome, salario FROM atleta WHERE salario BETWEEN 15000 and 25000;

SELECT cpf, nome FROM atleta WHERE id_clube is null ORDER BY nome asc;
...
SELECT CONCAT(nome,data_fundacao) from clube;

SELECT nome||data_fundacao from clube;

select SUBSTR(nome, 1,5) from atleta;
select SUBSTR(nome, -4,4) from clube;
select SUBSTR(nome, -4) from clube;

select nome, LENGTH(nome) as TAM from atleta;
select ROUND(salario,0), TRUNC(salario,1) from atleta;
select TRIM(nome) from atleta;
select nome, REPLACE(nome, 'e', 'i') from clube;
select TRANSLATE(nome, 'aei', 'ouy') from clube;

select nome, NVL(id_clube, 0) from atleta;
select nome, NVL(TO_CHAR(id_clube), 'Não possui') ID_CLUBE from atleta;
...
select nome, salario,
    case when salario > 50000 then 0.3
...
select UPPER(nome), datanasc,
    case when datanasc < to_date('01/01/1990','dd/mm/yyyy') then 5000
...
select nome,
    replace(replace(endereco, 'Avenue', 'Av.'), 'Street', 'St.') as "Endereço", NVL(TO_CHAR(id_clube), 'Não possui') as clube, to_char(salario,'$999,999.00') as salario
from atleta;

select max(salario) from atleta;
select min(salario) from atleta;
...
select max(salario) from atleta where id_clube = 24;


select count(*) from atleta;
select count(id_clube) from atleta;
select * from atleta where id_clube is null;


select sexo, count(sexo) from atleta group by sexo;
select id_clube, sum(salario) from atleta group by id_clube order by id_clube;
select id_clube, avg(salario) as Media from atleta where id_clube in (12, 15, 24) group by id_clube;
select id_clube, sexo, round(avg(salario),2) as Media from atleta group by id_clube, sexo order by id_clube;
...
select id_clube, sum(salario) from atleta group by id_clube having sum(salario) > 200000 order by id_clube;
select count(*) from centro_treinamento;
select id_clube, count(*) from centro_treinamento group by id_clube;
select id_clube from centro_treinamento group by id_clube having count(*) = 1;
...
SELECT SUM(valor_premiacao) FROM participa WHERE id_campeonato = 19;
...
FROM clube c RIGHT JOIN presidente p
ON c.id_presidente = p.id;

SELECT c.nome, p.nome
FROM clube c, presidente p
WHERE c.id_presidente(+) = p.id;


SELECT c.nome, p.nome
FROM clube c FULL JOIN presidente p
ON c.id_presidente = p.id;
...
FROM atleta
WHERE id_clube IN (SELECT id FROM clube
                    WHERE nome IN ('Tresom Club', 'Zamit Team'));

SELECT nome
FROM presidente
WHERE id NOT IN (SELECT NVL(id_presidente,0) FROM clube
...
INSERT INTO v_atleta26 VALUES (199, 'Neymar', '999-88-7777', 25000.00, 26);
...
INSERT INTO v_atleta26 VALUES (200, 'Jase Barbosa', '111-22-3333', 45700.8, 26);
...
AS SELECT c.nome, m.descricao, NVL(SUM(p.valor_premiacao),0) AS tot_premiacao
FROM atleta a, esporte e, participa p, clube c, modalidade m
WHERE a.id = e.id_atleta AND e.registro_atleta = p.registro_atleta AND a.id_clube = c.id AND e.id_modalidade = m.id
...
BEGIN
     ...
     DBMS_OUTPUT.PUT_LINE('Nome: ' || v_nome || ', Data Nasc: ' || v_datanasc || ', Idade = ' || v_idade || ', sexo: ' || v_sexo);
...
DECLARE
    ...
    v_salario atleta.salario%type;
CURSOR c_atleta IS select id, nome, cpf, salario from atleta;
BEGIN
